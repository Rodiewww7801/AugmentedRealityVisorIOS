...
struct ARItemObjectValueView: View {
    @ObservedObject var arItemViewModel : ARItemViewModel
    
    var body: some View {
        VStack() {
            ARItemObjectValueUIViewRepresentable(viewModel: arItemViewModel)
        }
    }
}

class ARItemObjectValueUIView: UIViewController {
    ...
    private func setupChart() {
        guard viewModel.objectValue?.drawChart ?? false else {
    ...
    func formatDataSet(dataSet: LineChartDataSet) {
        let color = UIColor(.black).hexStringToUIColor(hex: viewModel.objectValue?.hexColor ?? defaultHexColor)
    ...
    private func setupTextView() {
        guard let objectValue = viewModel.objectValue else { return }
        let attributetString = NSMutableAttributedString(string: "\(objectValue.name): ",
                                                         attributes: [NSAttributedString.Key.font : UIFont.systemFont(ofSize: 18)])
        attributetString.append(NSAttributedString(string: "\(objectValue.value)",
                                                   attributes: [NSAttributedString.Key.font : UIFont.systemFont(ofSize: 18)]))
    ...
    func setupDescriptionText() {
        guard let objectValue = viewModel.objectValue else { return }
        guard let description =  viewModel.objectValue?.description else {
...
struct ARItemObjectValueUIViewRepresentable: UIViewControllerRepresentable {
    @ObservedObject var viewModel: ARItemViewModel
    
    func makeUIViewController(context: Context) -> some ARItemObjectValueUIView {
        let view = ARItemObjectValueUIView(viewModel: viewModel)
        return view
    }
